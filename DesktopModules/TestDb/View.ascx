    ...
    function getHelloWorld() {
        try {
            $.ajax({
                ...
                url: 'http://localhost:52732/DesktopModules/TestDb/API/Example/HelloWorld',
    ...
    function postGoodbyeWorld() {
        try {
            $.ajax({
                type: "POST",
                cache: false,
                url: 'http://localhost:52732/DesktopModules/TestDb/API/Example/GoodbyeWorld',
           //     beforeSend: sf.setModuleHeaders,
                data: ({ goodbye: true , name1: 'asdasdasd'}),
                success: function (result) {
                    alert(result);
                },
                error: function (xhr, status, error) {
